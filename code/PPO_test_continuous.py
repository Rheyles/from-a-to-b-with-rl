...
@torch.no_grad()
def run_timestamps(env, model, timestamps=1024, render=False,device="cpu"):
    ...
    for ts in range(timestamps):
        ...
        if render:
            ...
            buffer.append([model_input, action, mu, sigma, value, reward, terminated or truncated])
...
def get_losses(model, batch, epsilon, annealing, device="cpu"):
    ...
    _, new_mus, new_sigmas, new_values = model(states)
